...
pub fn link(input_file: &PathBuf, config: &LinkageConfig) -> Result<(), Error> {
    ...
    let command = if let Some(command) = config.linker_command {
        custom_command(input_file, command)
    }
    else {
...
fn hosta_command(input: &PathBuf, output: &PathBuf) -> Command {
    let command = Command::new("link.exe");
...
fn custom_command(input: &PathBuf, custom: Vec<String>) -> Command {
